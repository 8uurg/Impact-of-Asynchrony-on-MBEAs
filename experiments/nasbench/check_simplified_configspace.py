...
# based on the original example.py from nasbench301
...
configspace_path = os.path.join(current_dir, 'configspace_simplified.json')
with open(configspace_path, "r") as f:
    json_string = f.read()
    configspace = cs_json.read(json_string)
# configspace_config = configspace.sample_configuration()

configspace_default_config = configspace.get_default_configuration()
...
prediction_configspace_default = performance_model.predict(config=configspace_default_config, representation="configspace", with_noise=True)
...
runtime_configspace_default = runtime_model.predict(config=configspace_default_config, representation="configspace")
...
print("Configspace default architecture performance: %f, runtime %f" %(prediction_configspace_default, runtime_configspace_default))
