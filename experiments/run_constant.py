...
output_dir = Path(
    f"results/{experiment_name}-{get_git_revision_short_hash()}/constant_{approach_name}_single_{uid}"
)
...
def fn(genotype):
    return 0.0
problem = ealib.DiscreteObjectiveFunction(fn, l, [chr(2) for _ in range(l)], )
limiter = ealib.Limiter(problem, 10_000)
