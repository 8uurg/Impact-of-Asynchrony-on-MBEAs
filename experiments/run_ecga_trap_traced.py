...
argparser.add_argument("num_processors", type=int)
...
num_processors = parsed.num_processors
do_log = True

max_generations = 500  # note: for async this gets multiplied by population size!

output_dir = Path(f"results/{experiment_name}-{get_git_revision_short_hash()}/trap_bisect_{uid}")
...
def run(population_size: int):
    ...
    limiter = ealib.Limiter(problem, int(1e7))
    ...
    elif runtime_type == "cheap-ones":

        def get_runtime(population, individual):
            a = np.array(
                population.getData(ealib.GENOTYPECATEGORICAL, individual), copy=False
            )
            return 2.0 - a.sum() / a.shape[0]
    ...
    elif runtime_type == "cheap-ones-10":

        def get_runtime(population, individual):
            a = np.array(
                population.getData(ealib.GENOTYPECATEGORICAL, individual), copy=False
            )
            return 10.0 - 9 * a.sum() / a.shape[0]
    ...
    elif runtime_type == "cheap-ones-100":

        def get_runtime(population, individual):
            a = np.array(
                population.getData(ealib.GENOTYPECATEGORICAL, individual), copy=False
            )
            return 100.0 - 99 * a.sum() / a.shape[0]
    ...
    if do_log:
        ...
        print(f"Got {-objectives[0]} wanted {vtr}")
    return successful
